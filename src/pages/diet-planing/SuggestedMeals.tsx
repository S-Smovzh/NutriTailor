    ...
    (async () => {
      try {
        ...
        if (filterType === FilterType.USE_PRODUCTS && products.length) {
          response = await apiRequest(`/ai-suggestions/mealCategory=${filterDish}/with-products`, { method: 'POST', timeout: 50000, data: products });
        } else {
          response = await apiRequest(`/ai-suggestions/mealCategory=${filterDish}`, { timeout: 50000 });
        }
    ...
    <Col>
      ...
      {meals &&
        ...
        meals.map((meal, index) => (
          <div key={meal._id} className="mt-4">
            <h1>Meal #{index + 1}</h1>
